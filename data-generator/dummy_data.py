...
def run_iteration():
    ...
    for _ in range(NUM_CUSTOMERS):
        ...
        cur.execute(
            "INSERT INTO customers (firstname, lastname, email) VALUES (%s, %s, %s) RETURNING customer_id",
    ...
    for _ in range(NUM_TRANSACTIONS):
        ...
        if txn_type == "TRANSFER" and len(accounts) > 1:
            recipient_account = random.choice([a for a in accounts if a != account_id])

        cur.execute(
            "INSERT INTO transactions (account_id, transaction_type, amount, recipient_account_id, status) VALUES (%s, %s, %s, %s, 'COMPLETED')",
            (account_id, txn_type, amount, recipient_account),
        )
